export interface DistrictInfo {
  name: string;
  postalCode: string;
  slug: string;
  description: string;
  metaDescription: string;
  shortSummary: string;
  landmarks: string[];
  neighborhoods: string[];
  characteristics: string[];
  serviceAreas: string[];
  popularServices: string[];
  whyChooseUs: string[];
  faq: Array<{ question: string; answer: string }>;
}

export const viennaDistricts: Record<string, DistrictInfo> = {
  '1010-wien-innere-stadt': {
    name: 'Innere Stadt',
    postalCode: '1010',
    slug: '1010-wien-innere-stadt',
    description: 'Die Innere Stadt ist das historische und kulturelle Herz Wiens. Als UNESCO-Weltkulturerbe beherbergt der 1. Bezirk weltberühmte Sehenswürdigkeiten wie den Stephansdom, die Hofburg und die Staatsoper.',
    metaDescription: 'Entrümpelung 1010 Wien Innere Stadt ✓ UNESCO-Weltkulturerbe ✓ Denkmalgeschützte Altbauten ✓ Diskrete Geschäftsentrümpelung am Graben ☎ +43 660 200 5610',
    shortSummary: 'Professionelle Entrümpelung im historischen 1. Bezirk. Diskrete Arbeit in UNESCO-Weltkulturerbe mit Erfahrung in denkmalgeschützten Gebäuden.',
    landmarks: ['Stephansdom', 'Hofburg', 'Staatsoper', 'Graben', 'Ringstraße', 'Burgtheater'],
    neighborhoods: ['Stubenviertel', 'Freihausviertel', 'Schottenviertel', 'Franziskanerviertel'],
    characteristics: [
      'UNESCO-Weltkulturerbe',
      'Historische Altbauten und Geschäftsgebäude',
      'Zentrum der Stadt mit hoher Bebauungsdichte',
      'Viele Geschäftsräume und Büros',
      'Luxuswohnungen und Penthäuser',
    ],
    serviceAreas: ['Stephansplatz', 'Graben', 'Kärntner Straße', 'Ringstraße', 'Herrengasse', 'Wollzeile'],
    popularServices: [
      'Büro- und Geschäftsentrümpelung',
      'Wohnungsentrümpelung in Altbauten',
      'Penthouse-Räumung',
      'Kellerentrümpelung',
      'Dachbodenentrümpelung',
      'Geschäftsauflösung',
    ],
    whyChooseUs: [
      'Spezialisiert auf UNESCO-Weltkulturerbe-Gebäude seit 15 Jahren',
      'Neutrale Fahrzeuge für diskrete Arbeit am Graben und Kohlmarkt',
      'Kenntnis aller Fußgängerzonenzufahrten und Sondergenehmigungsverfahren',
      'Entrümpelung außerhalb der Geschäftszeiten für Wiener Luxusboutiquen',
      'Expertise im Umgang mit Stuckdecken, Parkettböden und Wendeltreppen',
    ],
    faq: [
      {
        question: 'Können Sie auch in denkmalgeschützten Gebäuden entrümpeln?',
        answer: 'Selbstverständlich. Unser Team kennt die speziellen Anforderungen von Gründerzeithäusern in der Inneren Stadt - von schmalen Wendeltreppen bis zu historischen Stuckdecken. Wir schützen alle Oberflächen während der Entrümpelung.',
      },
      {
        question: 'Wie lange dauert eine Entrümpelung im 1. Bezirk?',
        answer: 'Eine typische 80m² Altbauwohnung im Franziskanerviertel entrümpeln wir in 5-7 Stunden. Bei Penthäusern am Graben planen wir 8-12 Stunden ein, abhängig von der Stockwerkshöhe und Objektmenge.',
      },
      {
        question: 'Gibt es Zufahrtsbeschränkungen in der Altstadt?',
        answer: 'Für die Fußgängerzone (Graben, Kärntner Straße, Kohlmarkt) organisieren wir Zufahrtsgenehmigungen für Zeitfenster 6-10 Uhr. In der Herrengasse und Wollzeile haben wir ganztägig Zugang mit kleineren Fahrzeugen.',
      },
    ],
  },
  '1020-wien-leopoldstadt': {
    name: 'Leopoldstadt',
    postalCode: '1020',
    slug: '1020-wien-leopoldstadt',
    description: 'Die Leopoldstadt ist der zweitgrößte Bezirk Wiens und bekannt für den Prater und das moderne Stadtviertel rund um den Praterstern. Der Bezirk verbindet historischen Charme mit modernem Stadtleben.',
    metaDescription: 'Entrümpelung 1020 Leopoldstadt ✓ Nordbahnviertel ✓ Prater ✓ Taborstraße ✓ Großobjekte & Gewerbe vom Praterstern bis Augarten ☎ +43 660 200 5610',
    shortSummary: 'Entrümpelung im 2. Bezirk von Prater bis Nordbahnviertel. Schnelle Erreichbarkeit vom Praterstern und Erfahrung mit Gewerbeobjekten.',
    landmarks: ['Prater', 'Riesenrad', 'Augarten', 'Praterstern', 'Messe Wien', 'Ernst-Happel-Stadion'],
    neighborhoods: ['Prater', 'Stuwerviertel', 'Nordbahnviertel', 'Volkertviertel', 'Mazzesinsel'],
    characteristics: [
      'Mix aus Altbau und Neubauten',
      'Große Grünflächen (Prater, Augarten)',
      'Wachsendes Stadtviertel Nordbahnhof',
      'Internationale Bevölkerung',
      'Viele Gewerbeflächen',
    ],
    serviceAreas: ['Praterstern', 'Taborstraße', 'Nordbahnstraße', 'Lassallestraße', 'Prater', 'Augarten'],
    popularServices: [
      'Wohnungsentrümpelung',
      'Kellerentrümpelung',
      'Garageentrümpelung im Prater-Gebiet',
      'Geschäftsentrümpelung',
      'Büroräumung',
      'Verlassenschaftsentrümpelung',
    ],
    whyChooseUs: [
      'Direkte Anfahrt vom Praterstern in unter 8 Minuten zu jedem Standort',
      'Entrümpelung von Gewerbeflächen bis 500m² in der Messe Wien Umgebung',
      'Spezialfahrzeuge für das enge Stuwerviertel und Mazzesinsel',
      'Entrümpelung rund um die Uhr für Gastronomie am Nordbahnhof',
      'Erfahrung mit modernen Neubauwohnungen im Nordbahnviertel seit 2015',
    ],
    faq: [
      {
        question: 'Entrümpeln Sie auch im Nordbahnviertel?',
        answer: 'Absolut! Das Nordbahnviertel ist einer unserer Haupteinsatzorte. Wir kennen alle Zufahrten, Tiefgaragen und die speziellen Anforderungen der modernen Neubauten. Für Wohnungen in den Bruno-Marek-Allee-Hochhäusern haben wir Express-Teams.',
      },
      {
        question: 'Können Sie auch Garagen im Prater-Gebiet räumen?',
        answer: 'Ja, speziell für das Prater-Gebiet haben wir kleinere Transporter für enge Garagenzufahrten. Wir räumen regelmäßig Lagerboxen bei der Messe Wien und in den Praterkellern - auch feuchte, vollgestellte Kellerabteile.',
      },
      {
        question: 'Wie schnell können Sie kommen?',
        answer: 'Von unserem Stützpunkt beim Praterstern erreichen wir jeden Punkt im 2. Bezirk in maximal 15 Minuten. Express-Einsätze in der Taborstraße oder am Augarten sind oft noch am selben Tag möglich.',
      },
    ],
  },
  '1030-wien-landstrasse': {
    name: 'Landstraße',
    postalCode: '1030',
    slug: '1030-wien-landstrasse',
    description: 'Der 3. Bezirk Landstraße ist geprägt von prächtigen Botschaftsgebäuden, dem barocken Schloss Belvedere und modernen Bürokomplexen. Ein gehobener Wohnbezirk mit ausgezeichneter Infrastruktur.',
    metaDescription: 'Entrümpelung 1030 Landstraße ✓ Botschaftsviertel Rennweg ✓ Diplomatenviertel ✓ Wien Mitte ✓ Diskret für gehobene Altbauten ☎ +43 660 200 5610',
    shortSummary: 'Diskrete Entrümpelung im gehobenen 3. Bezirk. Botschaftsviertel, Belvedere-Gegend mit Erfahrung in hochwertigen Immobilien.',
    landmarks: ['Schloss Belvedere', 'Wien Mitte', 'Stadtpark', 'Hundertwasserhaus', 'Botanischer Garten'],
    neighborhoods: ['Botschaftsviertel', 'Erdberg', 'Rennweg', 'Landstraßer Hauptstraße', 'Weißgerber'],
    characteristics: [
      'Gehobene Wohngegend',
      'Viele Botschaften und Konsulate',
      'Historische Palais und moderne Büros',
      'Ausgezeichnete Verkehrsanbindung',
      'Hochwertige Altbausubstanz',
    ],
    serviceAreas: ['Landstraßer Hauptstraße', 'Rennweg', 'Arsenal', 'Erdberg', 'Wien Mitte', 'Stadtpark'],
    popularServices: [
      'Wohnungsentrümpelung in Altbauten',
      'Büroräumung',
      'Kellerentrümpelung',
      'Dachbodenentrümpelung',
      'Verlassenschaftsentrümpelung',
      'Geschäftsauflösung',
    ],
    whyChooseUs: [
      'Absolute Diskretion für Botschaften am Rennweg und diplomatische Residenzen',
      'Wertgegenständeversicherung bis 50.000€ für Belvedere-Palais-Wohnungen',
      'Geräuscharme Entrümpelung in Bürokomplexen Wien Mitte während Öffnungszeiten',
      'Spezialisiert auf Jugendstil-Altbauten im Weißgerberviertel seit 2008',
      'Firmenwagen ohne Logo für unerkannte Entrümpelung im Diplomatenviertel',
    ],
    faq: [
      {
        question: 'Wie diskret arbeiten Sie in gehobenen Wohnlagen?',
        answer: 'Im Botschaftsviertel am Rennweg nutzen wir ausschließlich neutrale Limousinen und zivile Transporter ohne Firmenlogo. Unsere Mitarbeiter tragen Business-Kleidung, keine Arbeitsoveralls. Entrümpelungen in diplomatischen Gebäuden erfolgen nach Sicherheitsprotokoll mit Voranmeldung.',
      },
      {
        question: 'Können Sie Wertgegenstände sicher handhaben?',
        answer: 'Für hochwertige Immobilien im Belvedere-Viertel bieten wir eine separate Wertgegenständelinie an. Kunstwerke, Antiquitäten und Schmuck werden dokumentiert, fotografiert und in verschlossenen Boxen transportiert. Unsere Mitarbeiter sind versichert bis 50.000€.',
      },
      {
        question: 'Arbeiten Sie auch am Wochenende?',
        answer: 'Speziell für Wien Mitte und das Botschaftsviertel bieten wir Samstag-Termine an, wenn diskrete Entrümpelungen außerhalb der Bürozeiten gewünscht werden. Sonntags nur bei dringenden Verlassenschaftsräumungen.',
      },
    ],
  },
  '1040-wien-wieden': {
    name: 'Wieden',
    postalCode: '1040',
    slug: '1040-wien-wieden',
    description: 'Wieden ist ein lebendiger, zentrumsnaher Bezirk mit vielen Altbauten, der Karlskirche und dem beliebten Naschmarkt. Eine perfekte Mischung aus Wohnen, Arbeiten und Kultur.',
    metaDescription: 'Entrümpelung 1040 Wieden ✓ Altbau-Expertise Paulanerviertel ✓ Naschmarkt-Gebiet ✓ Stiegenhäuser ohne Lift bis 5. Stock ☎ +43 660 200 5610',
    shortSummary: 'Professionelle Entrümpelung im 4. Bezirk nahe Naschmarkt und Karlskirche. Erfahrung mit Altbau-Stiegenhäusern ohne Lift.',
    landmarks: ['Karlskirche', 'Naschmarkt', 'Karlsplatz', 'Resselpark', 'Wiedner Hauptstraße'],
    neighborhoods: ['Freihausviertel', 'Paulanerviertel', 'Schaumburggasse', 'Favoritenstraße'],
    characteristics: [
      'Zentrale Lage',
      'Historische Altbausubstanz',
      'Kulturelle Vielfalt',
      'Viele kleine Geschäfte und Lokale',
      'Gut erhaltene Gründerzeithäuser',
    ],
    serviceAreas: ['Wiedner Hauptstraße', 'Margaretenstraße', 'Favoritenstraße', 'Naschmarkt', 'Karlsplatz'],
    popularServices: [
      'Wohnungsentrümpelung',
      'Altbau-Kellerentrümpelung',
      'Geschäftsentrümpelung',
      'Dachbodenentrümpelung',
      'Haushaltsauflösung',
      'Verlassenschaftsentrümpelung',
    ],
    whyChooseUs: [
      'Tragen von Möbeln bis 5. Stock ohne Lift - täglich in Wiedner Gründerzeithäusern',
      'Zufahrt zu verwinkelten Innenhöfen im Freihausviertel seit 18 Jahren',
      'Entrümpelung während Naschmarkt-Öffnungszeiten für Gastro-Lokale',
      'Samstagstermine für Berufstätige rund um Karlsplatz besonders nachgefragt',
      'Fixpreise auch für verwinkelte Altbauwohnungen im Paulanerviertel',
    ],
    faq: [
      {
        question: 'Können Sie auch ohne Lift arbeiten?',
        answer: 'Im 4. Bezirk gibt es viele Gründerzeithäuser ohne Aufzug - das ist unser tägliches Geschäft. Unsere Teams tragen routiniert Möbel bis zum 5. Stock über enge Wendeltreppen. Für Klaviere und Schränke über 100kg nutzen wir Tragegurte und Möbelrollen.',
      },
      {
        question: 'Was kostet eine Entrümpelung im 4. Bezirk?',
        answer: 'Im Naschmarkt-Gebiet kalkulieren wir ab 600€ für Kleinentrümpelungen. Eine 70m² Altbauwohnung im Paulanerviertel kostet durchschnittlich 1.200-1.800€. Wir erstellen nach telefonischer Vorbesprechung verbindliche Festpreisangebote.',
      },
      {
        question: 'Wie lange dauert eine Wohnungsentrümpelung?',
        answer: 'Eine 70m² Altbauwohnung im Paulanerviertel ohne Lift räumen wir in 5-7 Stunden. Bei Wohnungen am Karlsplatz mit Lift sind wir schneller - meist 3-5 Stunden. Messie-Wohnungen am Naschmarkt benötigen erfahrungsgemäß 8-12 Stunden.',
      },
    ],
  },
  '1050-wien-margareten': {
    name: 'Margareten',
    postalCode: '1050',
    slug: '1050-wien-margareten',
    description: 'Margareten ist ein bunter, lebendiger Bezirk mit multikulturellen Einflüssen, vielen Lokalen und dem beliebten Margaretenplatz. Traditionelle Altbauten prägen das Stadtbild.',
    metaDescription: 'Entrümpelung 1050 Wien Margareten ✓ Reinprechtsdorfer Straße ✓ Margaretenplatz. Schnell & günstig. Jetzt anrufen ☎ +43 660 200 5610',
    shortSummary: 'Faire Entrümpelung im 5. Bezirk mit Verständnis für besondere Situationen. Diskrete Messie-Entrümpelungen und soziale Preisgestaltung.',
    landmarks: ['Margaretenplatz', 'Rüdigerhof', 'Matzleinsdorfer Platz'],
    neighborhoods: ['Hundsturm', 'Arbeiterviertel', 'Reinprechtsdorf', 'Laurenzberg'],
    characteristics: [
      'Dichte Wohnbebauung',
      'Traditionelle Arbeiterbezirk',
      'Viele Altbauten',
      'Multikulturell geprägt',
      'Lebendiges Grätzl-Leben',
    ],
    serviceAreas: ['Margaretenstraße', 'Reinprechtsdorfer Straße', 'Schönbrunner Straße', 'Matzleinsdorfer Platz'],
    popularServices: [
      'Wohnungsentrümpelung',
      'Kellerentrümpelung',
      'Haushaltsauflösung',
      'Messie-Entrümpelung',
      'Dachbodenentrümpelung',
      'Geschäftsentrümpelung',
    ],
    whyChooseUs: [
      'Soziale Preisgestaltung',
      'Verständnis für besondere Situationen',
      'Diskrete Messie-Entrümpelungen',
      'Schnelle Terminvergabe',
      'Erfahrene Teams',
    ],
    faq: [
      {
        question: 'Bieten Sie auch Messie-Entrümpelungen an?',
        answer: 'Ja, wir haben viel Erfahrung mit sensiblen Situationen und arbeiten diskret und respektvoll.',
      },
      {
        question: 'Können Sie auch kurzfristig kommen?',
        answer: 'Ja, oft können wir innerhalb von 1-2 Tagen einen Termin ermöglichen.',
      },
      {
        question: 'Was passiert mit dem Entrümpelungsgut?',
        answer: 'Wir entsorgen fachgerecht und führen brauchbare Gegenstände sozialen Einrichtungen zu.',
      },
    ],
  },
  '1060-wien-mariahilf': {
    name: 'Mariahilf',
    postalCode: '1060',
    slug: '1060-wien-mariahilf',
    description: 'Mariahilf ist ein urbaner Szenebezirk mit der Mariahilfer Straße als Shopping-Meile, vielen Cafés, Bars und einer lebendigen Kulturszene. Ein beliebter Wohnbezirk mit Altbaucharme.',
    metaDescription: 'Entrümpelung 1060 Wien Mariahilf ✓ Mariahilfer Straße ✓ Naschmarkt ✓ Professionell & schnell. Kostenlose Besichtigung ☎ +43 660 200 5610',
    shortSummary: 'Schnelle Entrümpelung im urbanen 6. Bezirk. Mariahilfer Straße und Umgebung mit flexiblen Arbeitszeiten für Geschäftskunden.',
    landmarks: ['Mariahilfer Straße', 'Haus des Meeres', 'Theater an der Wien', 'Naschmarkt'],
    neighborhoods: ['Gumpendorf', 'Windmühle', 'Laimgrube', 'Magdalenengrund'],
    characteristics: [
      'Zentrale Einkaufsstraße',
      'Lebendige Szene',
      'Historische Altbauten',
      'Hohe Wohndichte',
      'Kulturelle Vielfalt',
    ],
    serviceAreas: ['Mariahilfer Straße', 'Gumpendorfer Straße', 'Stumpergasse', 'Linke Wienzeile', 'Webgasse'],
    popularServices: [
      'Wohnungsentrümpelung',
      'Geschäftsentrümpelung',
      'Kellerentrümpelung',
      'Dachbodenentrümpelung',
      'Büroräumung',
      'Verlassenschaftsentrümpelung',
    ],
    whyChooseUs: [
      'Erfahrung in belebten Einkaufsstraßen',
      'Flexible Arbeitszeiten',
      'Schnelle Abwicklung',
      'Professionelle Teams',
      'Faire Preise',
    ],
    faq: [
      {
        question: 'Können Sie auch in der Mariahilfer Straße arbeiten?',
        answer: 'Ja, wir kennen die Zufahrtsmöglichkeiten und arbeiten effizient auch in belebten Gebieten.',
      },
      {
        question: 'Arbeiten Sie auch abends?',
        answer: 'Ja, wir bieten flexible Arbeitszeiten an, um Ihren Bedürfnissen gerecht zu werden.',
      },
      {
        question: 'Wie entsorgen Sie in der Stadt?',
        answer: 'Wir haben direkte Zufahrten zu Entsorgungsstellen und arbeiten effizient.',
      },
    ],
  },
  '1070-wien-neubau': {
    name: 'Neubau',
    postalCode: '1070',
    slug: '1070-wien-neubau',
    description: 'Neubau ist der kreative und hippe Bezirk Wiens mit vielen Designläden, Galerien, Cafés und dem MuseumsQuartier. Ein lebendiger Bezirk mit hoher Lebensqualität.',
    metaDescription: 'Entrümpelung 1070 Wien Neubau ✓ MuseumsQuartier ✓ Spittelberg ✓ Schnell & professionell. Kostenlose Beratung ☎ +43 660 200 5610',
    shortSummary: 'Professionelle Entrümpelung im kreativen 7. Bezirk. MuseumsQuartier, Spittelberg mit sorgfältigem Umgang mit Kunstobjekten.',
    landmarks: ['MuseumsQuartier', 'Spittelberg', 'Volkstheater', 'Neubaugasse'],
    neighborhoods: ['Spittelberg', 'Schottenfeld', 'Neubaugasse', 'Westbahnhof'],
    characteristics: [
      'Kreative Szene',
      'Viele Galerien und Designer-Shops',
      'Historische Biedermeier-Viertel',
      'Hohe Lebensqualität',
      'Kulturelles Zentrum',
    ],
    serviceAreas: ['Neubaugasse', 'Burggasse', 'Lerchenfelder Straße', 'Westbahnstraße', 'Siebensterngasse'],
    popularServices: [
      'Wohnungsentrümpelung',
      'Altbau-Kellerentrümpelung',
      'Geschäftsentrümpelung',
      'Atelier-Räumung',
      'Dachbodenentrümpelung',
      'Verlassenschaftsentrümpelung',
    ],
    whyChooseUs: [
      'Verständnis für kreative Räume',
      'Sorgfältiger Umgang mit Kunstobjekten',
      'Diskrete Arbeitsweise',
      'Erfahrung mit Altbauten',
      'Flexible Termingestaltung',
    ],
    faq: [
      {
        question: 'Können Sie auch Ateliers entrümpeln?',
        answer: 'Ja, wir haben Erfahrung mit Ateliers und gehen sorgfältig mit Kunstwerken um.',
      },
      {
        question: 'Wie gehen Sie mit wertvollen Gegenständen um?',
        answer: 'Wir behandeln alle Gegenstände mit Sorgfalt und können Wertvolles nach Wunsch separat handhaben.',
      },
      {
        question: 'Entrümpeln Sie auch am Spittelberg?',
        answer: 'Ja, wir sind im gesamten 7. Bezirk tätig, auch in engen Gassen wie am Spittelberg.',
      },
    ],
  },
  '1080-wien-josefstadt': {
    name: 'Josefstadt',
    postalCode: '1080',
    slug: '1080-wien-josefstadt',
    description: 'Die Josefstadt ist der kleinste und feinste Bezirk Wiens mit historischen Bürgerhäusern, dem berühmten Josefstädter Theater und einer gehobenen Wohnlage in Zentrumsnähe.',
    metaDescription: 'Entrümpelung 1080 Wien Josefstadt ✓ Josefstädter Straße ✓ Rathaus ✓ Diskret & professionell. Kostenlose Besichtigung ☎ +43 660 200 5610',
    shortSummary: 'Diskrete Entrümpelung im kleinsten Bezirk Wiens. Historische Bürgerhäuser mit Erfahrung in gehobenen Wohnlagen.',
    landmarks: ['Theater in der Josefstadt', 'Rathaus', 'Piaristenkirche'],
    neighborhoods: ['Josefstadt', 'Breitenfeld', 'Albertplatz', 'Strozzigasse'],
    characteristics: [
      'Kleinster Bezirk Wiens',
      'Gehobene Wohnlage',
      'Historische Bürgerhäuser',
      'Zentrale Lage',
      'Hochwertige Altbausubstanz',
    ],
    serviceAreas: ['Josefstädter Straße', 'Lerchenfelder Straße', 'Florianigasse', 'Albertgasse', 'Bennoplatz'],
    popularServices: [
      'Wohnungsentrümpelung',
      'Kellerentrümpelung',
      'Dachbodenentrümpelung',
      'Verlassenschaftsentrümpelung',
      'Büroräumung',
      'Haushaltsauflösung',
    ],
    whyChooseUs: [
      'Diskrete Arbeit in gehobener Wohnlage',
      'Erfahrung mit historischen Gebäuden',
      'Sorgfältiger Umgang mit Immobilien',
      'Pünktlichkeit und Professionalität',
      'Faire Preisgestaltung',
    ],
    faq: [
      {
        question: 'Arbeiten Sie auch in denkmalgeschützten Häusern?',
        answer: 'Ja, wir haben umfangreiche Erfahrung mit historischen und denkmalgeschützten Gebäuden.',
      },
      {
        question: 'Wie diskret arbeiten Sie?',
        answer: 'Wir arbeiten äußerst diskret mit neutralen Fahrzeugen und in Absprache mit Ihnen.',
      },
      {
        question: 'Können Sie auch ohne Lift entrümpeln?',
        answer: 'Selbstverständlich, unsere Teams sind erfahren im Tragen über Stiegen.',
      },
    ],
  },
  '1090-wien-alsergrund': {
    name: 'Alsergrund',
    postalCode: '1090',
    slug: '1090-wien-alsergrund',
    description: 'Der Alsergrund ist ein traditionsreicher Bezirk mit der Universität Wien, dem AKH und vielen medizinischen Einrichtungen. Ein lebendiger Studentenbezirk mit historischem Flair.',
    metaDescription: 'Entrümpelung 1090 Wien Alsergrund ✓ Universität Wien ✓ AKH ✓ Schnell & günstig. Kostenlose Beratung ☎ +43 660 200 5610',
    shortSummary: 'Günstige Entrümpelung im Universitätsviertel. Studenten-freundliche Preise und Erfahrung mit medizinischen Einrichtungen.',
    landmarks: ['Universität Wien', 'Allgemeines Krankenhaus', 'Sigmund Freud Museum', 'Votivkirche'],
    neighborhoods: ['Rossau', 'Lichtental', 'Althangrund', 'Servitenviertel'],
    characteristics: [
      'Universitätsviertel',
      'Viele Studenten',
      'Medizinisches Zentrum',
      'Historische Altbauten',
      'Kulturelle Vielfalt',
    ],
    serviceAreas: ['Währinger Straße', 'Nußdorfer Straße', 'Alser Straße', 'Spitalgasse', 'Rossauer Lände'],
    popularServices: [
      'Wohnungsentrümpelung',
      'Studenten-Wohnungsentrümpelung',
      'Kellerentrümpelung',
      'Praxisräumung',
      'Büroräumung',
      'Verlassenschaftsentrümpelung',
    ],
    whyChooseUs: [
      'Günstige Preise für Studenten',
      'Schnelle Abwicklung',
      'Erfahrung mit medizinischen Einrichtungen',
      'Flexible Termine',
      'Professionelle Entsorgung',
    ],
    faq: [
      {
        question: 'Gibt es Sonderpreise für Studenten?',
        answer: 'Wir bieten faire Preise und können individuell auf Ihre Situation eingehen.',
      },
      {
        question: 'Können Sie auch Arztpraxen räumen?',
        answer: 'Ja, wir haben Erfahrung mit medizinischen Einrichtungen und entsorgen fachgerecht.',
      },
      {
        question: 'Wie schnell können Sie kommen?',
        answer: 'Oft können wir innerhalb von 24-48 Stunden einen Termin anbieten.',
      },
    ],
  },
  '1100-wien-favoriten': {
    name: 'Favoriten',
    postalCode: '1100',
    slug: '1100-wien-favoriten',
    description: 'Favoriten ist der bevölkerungsreichste Bezirk Wiens mit dem Hauptbahnhof, vielfältigen Wohngebieten und dem Erholungsgebiet Laaer Berg. Ein dynamischer Bezirk im Wandel.',
    metaDescription: 'Entrümpelung 1100 Wien Favoriten ✓ Hauptbahnhof ✓ Laaer Berg ✓ Professionell & günstig. Kostenlose Besichtigung ☎ +43 660 200 5610',
    shortSummary: 'Entrümpelung im bevölkerungsreichsten Bezirk. Hauptbahnhof, Sonnwendviertel mit großer Fahrzeugflotte für große Objekte.',
    landmarks: ['Wien Hauptbahnhof', 'Laaer Berg', 'Favoriten-Straße', 'Therme Wien'],
    neighborhoods: ['Sonnwendviertel', 'Laaer Berg', 'Oberlaa', 'Unterlaa', 'Inzersdorf'],
    characteristics: [
      'Größter Bezirk nach Einwohnern',
      'Mix aus Alt- und Neubau',
      'Internationales Flair',
      'Hauptbahnhof als Verkehrsdrehscheibe',
      'Viele Gewerbebetriebe',
    ],
    serviceAreas: ['Favoritenstraße', 'Triester Straße', 'Quellenstraße', 'Laaer-Berg-Straße', 'Hauptbahnhof'],
    popularServices: [
      'Wohnungsentrümpelung',
      'Kellerentrümpelung',
      'Geschäftsentrümpelung',
      'Lagerentrümpelung',
      'Haushaltsauflösung',
      'Verlassenschaftsentrümpelung',
    ],
    whyChooseUs: [
      'Erfahrung mit großen Objekten',
      'Schnelle Anfahrt vom Hauptbahnhof',
      'Günstige Preise',
      'Große Fahrzeugflotte',
      'Flexible Termine',
    ],
    faq: [
      {
        question: 'Entrümpeln Sie auch im neuen Sonnwendviertel?',
        answer: 'Ja, wir sind im gesamten 10. Bezirk tätig, auch in den neuen Stadtteilen.',
      },
      {
        question: 'Können Sie auch große Gewerbeobjekte räumen?',
        answer: 'Ja, wir haben die Kapazität für große Objekte und Gewerbeflächen.',
      },
      {
        question: 'Was kostet eine Entrümpelung in Favoriten?',
        answer: 'Nach einer kostenlosen Besichtigung erhalten Sie einen fairen Festpreis.',
      },
    ],
  },
  '1110-wien-simmering': {
    name: 'Simmering',
    postalCode: '1110',
    slug: '1110-wien-simmering',
    description: 'Simmering ist ein großflächiger Bezirk im Südosten Wiens mit Industrie, Wohngebieten und viel Grünraum. Der Bezirk verbindet Arbeiten und Wohnen in aufstrebenden Vierteln.',
    metaDescription: 'Entrümpelung 1110 Wien Simmering ✓ Simmeringer Hauptstraße ✓ Schnell & zuverlässig. Kostenlose Beratung ☎ +43 660 200 5610',
    shortSummary: 'Gewerbeobjekt-Entrümpelung im 11. Bezirk. Lagerhallen, Industrie mit Erfahrung und großer Entsorgungskapazität.',
    landmarks: ['Simmeringer Hauptstraße', 'Zentralfriedhof', 'Gasometer', 'Haidepark'],
    neighborhoods: ['Simmering', 'Kaiserebersdorf', 'Albern', 'Schwechat'],
    characteristics: [
      'Industrie- und Gewerbebetriebe',
      'Gemischte Wohngebiete',
      'Viele Lagerhallen',
      'Gute Verkehrsanbindung',
      'Entwicklungsgebiet',
    ],
    serviceAreas: ['Simmeringer Hauptstraße', 'Braunhubergasse', 'Grillgasse', 'Eleven Vienna', 'Haidestraße'],
    popularServices: [
      'Lagerentrümpelung',
      'Gewerbeobjekt-Räumung',
      'Wohnungsentrümpelung',
      'Kellerentrümpelung',
      'Geschäftsentrümpelung',
      'Industrieentrümpelung',
    ],
    whyChooseUs: [
      'Erfahrung mit Gewerbeobjekten',
      'Große Entsorgungskapazität',
      'Flexible Arbeitszeiten',
      'Professionelle Abwicklung',
      'Faire Preise für große Objekte',
    ],
    faq: [
      {
        question: 'Können Sie auch Lagerhallen entrümpeln?',
        answer: 'Ja, wir haben umfangreiche Erfahrung mit großen Gewerbeobjekten und Lagerhallen.',
      },
      {
        question: 'Arbeiten Sie auch nachts?',
        answer: 'Bei Gewerbeobjekten können wir flexible Arbeitszeiten, auch nachts, anbieten.',
      },
      {
        question: 'Wie entsorgen Sie große Mengen?',
        answer: 'Wir haben direkte Partnerschaften mit Entsorgungsbetrieben für große Volumina.',
      },
    ],
  },
  '1120-wien-meidling': {
    name: 'Meidling',
    postalCode: '1120',
    slug: '1120-wien-meidling',
    description: 'Meidling ist ein traditioneller Arbeiterbezirk mit dem Einkaufszentrum Huma Eleven, der Schönbrunner Straße und vielfältigen Wohngegenden. Ein lebendiger Bezirk mit guter Infrastruktur.',
    metaDescription: 'Entrümpelung 1120 Wien Meidling ✓ Schönbrunner Straße ✓ Hetzendorf ✓ Professionell & fair. Kostenlose Besichtigung ☎ +43 660 200 5610',
    shortSummary: 'Faire Entrümpelung im traditionellen 12. Bezirk. Zentrale Verkehrslage mit schneller Anfahrt und lokalem Engagement.',
    landmarks: ['Huma Eleven', 'Schönbrunn', 'Gaudenzdorf', 'Meidlinger Markt'],
    neighborhoods: ['Hetzendorf', 'Altmannsdorf', 'Gaudenzdorf', 'Hetzendorf'],
    characteristics: [
      'Traditioneller Arbeiterbezirk',
      'Gute Einkaufsmöglichkeiten',
      'Mix aus Alt- und Neubau',
      'Internationale Bevölkerung',
      'Zentrale Verkehrslage',
    ],
    serviceAreas: ['Schönbrunner Straße', 'Meidlinger Hauptstraße', 'Hetzendorfer Straße', 'Gaudenzdorfer Gürtel'],
    popularServices: [
      'Wohnungsentrümpelung',
      'Kellerentrümpelung',
      'Geschäftsentrümpelung',
      'Haushaltsauflösung',
      'Dachbodenentrümpelung',
      'Verlassenschaftsentrümpelung',
    ],
    whyChooseUs: [
      'Lokale Verwurzelung',
      'Faire Preise',
      'Schnelle Abwicklung',
      'Soziales Engagement',
      'Erfahrene Teams',
    ],
    faq: [
      {
        question: 'Wie schnell können Sie in Meidling sein?',
        answer: 'Durch die zentrale Lage können wir sehr schnell vor Ort sein, oft innerhalb von 24 Stunden.',
      },
      {
        question: 'Können Sie auch größere Objekte entrümpeln?',
        answer: 'Ja, wir haben die Kapazität für Objekte jeder Größe.',
      },
      {
        question: 'Gibt es einen Mindestpreis?',
        answer: 'Nein, wir kalkulieren fair nach Aufwand. Auch kleine Entrümpelungen sind möglich.',
      },
    ],
  },
  '1130-wien-hietzing': {
    name: 'Hietzing',
    postalCode: '1130',
    slug: '1130-wien-hietzing',
    description: 'Hietzing ist einer der vornehmsten Bezirke Wiens mit Schloss Schönbrunn, dem Lainzer Tiergarten und noblen Villenvierteln. Eine grüne Oase mit höchster Lebensqualität.',
    metaDescription: 'Entrümpelung 1130 Wien Hietzing ✓ Schönbrunn ✓ Lainz ✓ Diskret & professionell. Kostenlose Beratung ☎ +43 660 200 5610',
    shortSummary: 'Exklusive Entrümpelung im nobelsten Bezirk. Villen, Herrschaftshäuser mit höchster Diskretion und Sorgfalt.',
    landmarks: ['Schloss Schönbrunn', 'Lainzer Tiergarten', 'Schönbrunner Schlosspark', 'Gloriette'],
    neighborhoods: ['Hietzing', 'Unter St. Veit', 'Ober St. Veit', 'Lainz', 'Speising'],
    characteristics: [
      'Nobelster Bezirk Wiens',
      'Villen und Herrschaftshäuser',
      'Größte Grünflächen',
      'Hohe Lebensqualität',
      'Gehobene Wohnlage',
    ],
    serviceAreas: ['Hietzinger Hauptstraße', 'Lainzer Straße', 'Auhofstraße', 'Maxingstraße', 'Schönbrunner Schlossstraße'],
    popularServices: [
      'Villen-Entrümpelung',
      'Herrschaftshaus-Räumung',
      'Wohnungsentrümpelung',
      'Kellerentrümpelung',
      'Verlassenschaftsentrümpelung',
      'Gartenentrümpelung',
    ],
    whyChooseUs: [
      'Höchste Diskretion',
      'Erfahrung mit gehobenen Objekten',
      'Sorgfältiger Umgang mit Wertgegenständen',
      'Professionelle Abwicklung',
      'Vertrauenswürdige Teams',
    ],
    faq: [
      {
        question: 'Wie diskret arbeiten Sie in Hietzing?',
        answer: 'Wir arbeiten äußerst diskret mit neutralen Fahrzeugen und in enger Absprache mit Ihnen.',
      },
      {
        question: 'Können Sie auch Villen entrümpeln?',
        answer: 'Ja, wir haben umfangreiche Erfahrung mit großen, hochwertigen Objekten.',
      },
      {
        question: 'Wie gehen Sie mit wertvollen Antiquitäten um?',
        answer: 'Wir behandeln alle Gegenstände mit größter Sorgfalt und können Experten hinzuziehen.',
      },
    ],
  },
  '1140-wien-penzing': {
    name: 'Penzing',
    postalCode: '1140',
    slug: '1140-wien-penzing',
    description: 'Penzing ist ein weitläufiger Bezirk im Westen Wiens mit dem Wienerwald, Schloss Schönbrunn und vielfältigen Wohngebieten. Viel Grün und hohe Lebensqualität prägen den Bezirk.',
    metaDescription: 'Entrümpelung 1140 Wien Penzing ✓ Hütteldorf ✓ Baumgarten ✓ Professionell & schnell. Kostenlose Besichtigung ☎ +43 660 200 5610',
    shortSummary: 'Entrümpelung im grünen 14. Bezirk. Hütteldorf bis Wienerwald mit Erfahrung in Gärten und Außenbereichen.',
    landmarks: ['Technisches Museum', 'Schloss Schönbrunn', 'Wienerwald', 'Dehnepark'],
    neighborhoods: ['Hütteldorf', 'Baumgarten', 'Breitensee', 'Hadikgasse', 'Hadersdorf'],
    characteristics: [
      'Weitläufiger Bezirk',
      'Viel Grünraum',
      'Mix aus Villen und Wohnhäusern',
      'Nähe zum Wienerwald',
      'Gute Lebensqualität',
    ],
    serviceAreas: ['Hütteldorfer Straße', 'Hadikgasse', 'Linzer Straße', 'Johnstraße', 'Felbigergasse'],
    popularServices: [
      'Wohnungsentrümpelung',
      'Kellerentrümpelung',
      'Dachbodenentrümpelung',
      'Gartenentrümpelung',
      'Hausräumung',
      'Verlassenschaftsentrümpelung',
    ],
    whyChooseUs: [
      'Kenntnis der weitläufigen Gebiete',
      'Erfahrung mit Gärten und Außenflächen',
      'Flexible Anfahrt',
      'Faire Preise',
      'Umweltbewusste Entsorgung',
    ],
    faq: [
      {
        question: 'Können Sie auch Gärten entrümpeln?',
        answer: 'Ja, wir entrümpeln auch Gärten, Schuppen und Außenbereiche.',
      },
      {
        question: 'Kommen Sie bis nach Hadersdorf?',
        answer: 'Ja, wir decken den gesamten 14. Bezirk ab, auch die Randlagen.',
      },
      {
        question: 'Wie entsorgen Sie Gartenabfälle?',
        answer: 'Wir entsorgen Gartenabfälle fachgerecht über spezialisierte Entsorgungsbetriebe.',
      },
    ],
  },
  '1150-wien-rudolfsheim-fuenfhaus': {
    name: 'Rudolfsheim-Fünfhaus',
    postalCode: '1150',
    slug: '1150-wien-rudolfsheim-fuenfhaus',
    description: 'Rudolfsheim-Fünfhaus ist ein lebendiger, multikultureller Bezirk mit der Mariahilfer Straße, dem Westbahnhof und traditionellen Wohnvierteln. Ein Bezirk im Aufbruch.',
    metaDescription: 'Entrümpelung 1150 Wien Rudolfsheim-Fünfhaus ✓ Westbahnhof ✓ Mariahilfer Straße ✓ Günstig & schnell ☎ +43 660 200 5610',
    shortSummary: 'Soziale Entrümpelung im multikulturellen 15. Bezirk. Westbahnhof-Nähe mit mehrsprachigen Teams und fairen Preisen.',
    landmarks: ['Westbahnhof', 'Mariahilfer Straße', 'Stadthalle'],
    neighborhoods: ['Rudolfsheim', 'Fünfhaus', 'Rustendorf', 'Sechshaus'],
    characteristics: [
      'Multikultureller Bezirk',
      'Traditionelle Wohnviertel',
      'Gute Verkehrsanbindung',
      'Lebendiges Grätzl-Leben',
      'Entwicklungsgebiet',
    ],
    serviceAreas: ['Mariahilfer Straße', 'Schmelz', 'Johnstraße', 'Hütteldorfer Straße', 'Westbahnstraße'],
    popularServices: [
      'Wohnungsentrümpelung',
      'Kellerentrümpelung',
      'Geschäftsentrümpelung',
      'Messie-Entrümpelung',
      'Haushaltsauflösung',
      'Verlassenschaftsentrümpelung',
    ],
    whyChooseUs: [
      'Soziale Preisgestaltung',
      'Verständnis für vielfältige Situationen',
      'Schnelle Terminvergabe',
      'Mehrsprachige Teams',
      'Lokale Verwurzelung',
    ],
    faq: [
      {
        question: 'Sprechen Ihre Teams auch andere Sprachen?',
        answer: 'Ja, unsere Teams sprechen mehrere Sprachen und können in verschiedenen Sprachen kommunizieren.',
      },
      {
        question: 'Können Sie auch bei schwierigen Situationen helfen?',
        answer: 'Ja, wir haben viel Erfahrung und arbeiten sensibel und professionell.',
      },
      {
        question: 'Wie schnell können Sie kommen?',
        answer: 'Oft können wir innerhalb von 1-2 Tagen vor Ort sein.',
      },
    ],
  },
  '1160-wien-ottakring': {
    name: 'Ottakring',
    postalCode: '1160',
    slug: '1160-wien-ottakring',
    description: 'Ottakring ist ein traditioneller Bezirk mit dem Brunnenmarkt, dem Wienerwald und gemütlichen Wohnvierteln. Multikulturell, lebendig und mit viel Charakter.',
    metaDescription: 'Entrümpelung 1160 Wien Ottakring ✓ Brunnenmarkt ✓ Wilhelminenspital ✓ Professionell & fair. Kostenlose Beratung ☎ +43 660 200 5610',
    shortSummary: 'Faire Entrümpelung im 16. Bezirk rund um den Brunnenmarkt. Lokale Kenntnis und verständnisvolle Teams.',
    landmarks: ['Brunnenmarkt', 'Wilhelminenspital', 'Ottakringer Brauerei', 'Gallitzinberg'],
    neighborhoods: ['Ottakring', 'Neulerchenfeld', 'Sandleiten'],
    characteristics: [
      'Traditioneller Arbeiterbezirk',
      'Größter Straßenmarkt Wiens',
      'Multikulturell',
      'Nähe zum Wienerwald',
      'Gemütliche Altbauviertel',
    ],
    serviceAreas: ['Brunnengasse', 'Ottakringer Straße', 'Thaliastraße', 'Wilhelminenstraße', 'Neulerchenfelder Straße'],
    popularServices: [
      'Wohnungsentrümpelung',
      'Kellerentrümpelung',
      'Geschäftsentrümpelung',
      'Haushaltsauflösung',
      'Dachbodenentrümpelung',
      'Verlassenschaftsentrümpelung',
    ],
    whyChooseUs: [
      'Lokale Kenntnis',
      'Faire, soziale Preise',
      'Verständnisvolle Teams',
      'Schnelle Abwicklung',
      'Mehrsprachigkeit',
    ],
    faq: [
      {
        question: 'Arbeiten Sie auch im Brunnenmarktviertel?',
        answer: 'Ja, wir kennen das Viertel gut und arbeiten effizient auch bei schwierigen Zufahrten.',
      },
      {
        question: 'Was kostet eine Entrümpelung in Ottakring?',
        answer: 'Nach kostenloser Besichtigung erhalten Sie einen fairen Festpreis.',
      },
      {
        question: 'Können Sie auch kurzfristig kommen?',
        answer: 'Ja, wir bemühen uns um schnelle Termine, oft innerhalb von 24-48 Stunden.',
      },
    ],
  },
  '1170-wien-hernals': {
    name: 'Hernals',
    postalCode: '1170',
    slug: '1170-wien-hernals',
    description: 'Hernals ist ein grüner Bezirk am Fuße des Wienerwalds mit dem Hernalser Gürtel, Dornbach und ruhigen Wohngebieten. Hohe Lebensqualität und Naturnähe zeichnen den Bezirk aus.',
    metaDescription: 'Entrümpelung 1170 Wien Hernals ✓ Dornbach ✓ Hernalser Gürtel ✓ Schnell & professionell. Kostenlose Besichtigung ☎ +43 660 200 5610',
    shortSummary: 'Grüne Entrümpelung im 17. Bezirk am Wienerwald. Hanglagen, Dornbach mit Erfahrung in Gärten und Außenbereichen.',
    landmarks: ['Alszeile', 'Dornbacher Park', 'Kongresspark Neuwaldegg'],
    neighborhoods: ['Hernals', 'Dornbach', 'Neuwaldegg'],
    characteristics: [
      'Grüner Bezirk',
      'Nähe zum Wienerwald',
      'Ruhige Wohngebiete',
      'Gute Luft',
      'Hohe Lebensqualität',
    ],
    serviceAreas: ['Hernalser Hauptstraße', 'Alszeile', 'Dornbacher Straße', 'Gürtel', 'Neuwaldegg'],
    popularServices: [
      'Wohnungsentrümpelung',
      'Kellerentrümpelung',
      'Dachbodenentrümpelung',
      'Hausräumung',
      'Gartenentrümpelung',
      'Verlassenschaftsentrümpelung',
    ],
    whyChooseUs: [
      'Kenntnis der Hanglagen',
      'Erfahrung mit Gärten',
      'Zuverlässige Anfahrt',
      'Umweltbewusste Entsorgung',
      'Faire Preise',
    ],
    faq: [
      {
        question: 'Können Sie auch in Hanglagen arbeiten?',
        answer: 'Ja, wir haben Erfahrung mit schwierigen Zugängen und Hanglagen.',
      },
      {
        question: 'Entrümpeln Sie auch bis Neuwaldegg?',
        answer: 'Ja, wir decken den gesamten 17. Bezirk ab, auch die Randlagen.',
      },
      {
        question: 'Wie gehen Sie mit Gartengeräten um?',
        answer: 'Wir entsorgen fachgerecht oder können brauchbare Geräte nach Wunsch separat behandeln.',
      },
    ],
  },
  '1180-wien-waehring': {
    name: 'Währing',
    postalCode: '1180',
    slug: '1180-wien-waehring',
    description: 'Währing ist ein vornehmer, grüner Bezirk mit Villen, dem Türkenschanzpark und der Währinger Straße. Eine gehobene Wohnlage mit exzellenter Infrastruktur.',
    metaDescription: 'Entrümpelung 1180 Wien Währing ✓ Türkenschanzpark ✓ Währinger Straße ✓ Diskret & professionell. Kostenlose Beratung ☎ +43 660 200 5610',
    shortSummary: 'Diskrete Villen-Entrümpelung im gehobenen 18. Bezirk. Türkenschanzpark-Gebiet mit höchster Professionalität.',
    landmarks: ['Türkenschanzpark', 'Schubertpark', 'Währinger Straße', 'Gersthofer Straße'],
    neighborhoods: ['Währing', 'Gersthof', 'Pötzleinsdorf', 'Weinhaus'],
    characteristics: [
      'Gehobener Bezirk',
      'Villen und Herrschaftshäuser',
      'Viele Parks',
      'Gute Schulen',
      'Hohe Lebensqualität',
    ],
    serviceAreas: ['Währinger Straße', 'Gersthofer Straße', 'Gymnasiumstraße', 'Gentzgasse', 'Kreuzgasse'],
    popularServices: [
      'Villen-Entrümpelung',
      'Wohnungsentrümpelung',
      'Kellerentrümpelung',
      'Dachbodenentrümpelung',
      'Verlassenschaftsentrümpelung',
      'Gartenentrümpelung',
    ],
    whyChooseUs: [
      'Höchste Diskretion',
      'Erfahrung mit gehobenen Immobilien',
      'Sorgfältiger Umgang',
      'Professionelle Teams',
      'Vertrauenswürdig',
    ],
    faq: [
      {
        question: 'Arbeiten Sie auch in Villengegenden?',
        answer: 'Ja, wir haben umfangreiche Erfahrung mit gehobenen Objekten und Villen.',
      },
      {
        question: 'Wie diskret arbeiten Sie?',
        answer: 'Wir arbeiten äußerst diskret und in enger Absprache mit Ihnen.',
      },
      {
        question: 'Können Sie Wertgegenstände schätzen?',
        answer: 'Wir können bei Bedarf Experten für Wertermittlungen hinzuziehen.',
      },
    ],
  },
  '1190-wien-doebling': {
    name: 'Döbling',
    postalCode: '1190',
    slug: '1190-wien-doebling',
    description: 'Döbling ist der vornehmste Bezirk Wiens mit Grinzing, dem Kahlenberg und noblen Villenvierteln. Weinberge, Heurige und höchste Lebensqualität prägen den Bezirk.',
    metaDescription: 'Entrümpelung 1190 Wien Döbling ✓ Grinzing ✓ Heiligenstadt ✓ Diskret & professionell. Kostenlose Besichtigung ☎ +43 660 200 5610',
    shortSummary: 'Exklusive Herrschaftshaus-Entrümpelung im vornehmsten Bezirk. Grinzing, Kahlenberg mit absoluter Diskretion.',
    landmarks: ['Grinzing', 'Kahlenberg', 'Heiligenstadt', 'Beethoven-Wohnhaus', 'Hohe Warte'],
    neighborhoods: ['Grinzing', 'Heiligenstadt', 'Nußdorf', 'Sievering', 'Oberdöbling'],
    characteristics: [
      'Vornehmster Bezirk',
      'Weinberge und Heurige',
      'Villen und Herrschaftshäuser',
      'Viel Grün',
      'Höchste Lebensqualität',
    ],
    serviceAreas: ['Döblinger Hauptstraße', 'Heiligenstädter Straße', 'Grinzinger Straße', 'Hohe Warte', 'Sieveringer Straße'],
    popularServices: [
      'Villen-Entrümpelung',
      'Herrschaftshaus-Räumung',
      'Wohnungsentrümpelung',
      'Weinkeller-Entrümpelung',
      'Verlassenschaftsentrümpelung',
      'Gartenentrümpelung',
    ],
    whyChooseUs: [
      'Absolute Diskretion',
      'Erfahrung mit Luxusimmobilien',
      'Sorgfältigster Umgang',
      'Höchste Professionalität',
      'Vertrauenswürdige Abwicklung',
    ],
    faq: [
      {
        question: 'Wie diskret arbeiten Sie in Döbling?',
        answer: 'Wir arbeiten mit höchster Diskretion, neutralen Fahrzeugen und nach Ihren Wünschen.',
      },
      {
        question: 'Können Sie auch Weinkeller entrümpeln?',
        answer: 'Ja, wir haben Erfahrung mit Weinkellern und gehen sorgfältig mit Weinflaschen um.',
      },
      {
        question: 'Arbeiten Sie auch in Hanglagen?',
        answer: 'Ja, wir sind auf die besonderen Anforderungen der Weinberggegend vorbereitet.',
      },
    ],
  },
  '1200-wien-brigittenau': {
    name: 'Brigittenau',
    postalCode: '1200',
    slug: '1200-wien-brigittenau',
    description: 'Die Brigittenau ist eine Donauinsel zwischen Donaukanal und Donau mit dem Millennium Tower, Handelskai und vielfältigen Wohngebieten. Ein Bezirk im Wandel.',
    metaDescription: 'Entrümpelung 1200 Wien Brigittenau ✓ Millennium Tower ✓ Handelskai ✓ Professionell & günstig. Kostenlose Beratung ☎ +43 660 200 5610',
    shortSummary: 'Professionelle Entrümpelung im 20. Bezirk zwischen Donau und Kanal. Millennium City, Handelskai mit flexiblen Terminen.',
    landmarks: ['Millennium Tower', 'Augarten', 'Donaukanal', 'Handelskai'],
    neighborhoods: ['Brigittenau', 'Zwischenbrücken'],
    characteristics: [
      'Insel zwischen Donau und Kanal',
      'Mix aus Alt und Neu',
      'Aufstrebendes Viertel',
      'Nähe zur Donau',
      'Multikulturell',
    ],
    serviceAreas: ['Brigittenauer Lände', 'Handelskai', 'Wallensteinstraße', 'Dresdner Straße', 'Millennium City'],
    popularServices: [
      'Wohnungsentrümpelung',
      'Kellerentrümpelung',
      'Geschäftsentrümpelung',
      'Büroräumung',
      'Haushaltsauflösung',
      'Verlassenschaftsentrümpelung',
    ],
    whyChooseUs: [
      'Kenntnis der Insel-Topografie',
      'Schnelle Anfahrt',
      'Faire Preise',
      'Flexible Termine',
      'Professionelle Abwicklung',
    ],
    faq: [
      {
        question: 'Kommen Sie auch zum Handelskai?',
        answer: 'Ja, wir decken den gesamten 20. Bezirk ab, auch die Donauufer-Bereiche.',
      },
      {
        question: 'Können Sie auch Büros entrümpeln?',
        answer: 'Ja, wir haben Erfahrung mit Büroräumungen und Geschäftsentrümpelungen.',
      },
      {
        question: 'Wie schnell können Sie kommen?',
        answer: 'Oft können wir innerhalb von 24-48 Stunden einen Termin anbieten.',
      },
    ],
  },
  '1210-wien-floridsdorf': {
    name: 'Floridsdorf',
    postalCode: '1210',
    slug: '1210-wien-floridsdorf',
    description: 'Floridsdorf ist der zweitgrößte Bezirk Wiens nördlich der Donau mit der Alten Donau, Stammersdorf und vielfältigen Wohn- und Erholungsgebieten. Ländlicher Charme trifft auf urbanes Leben.',
    metaDescription: 'Entrümpelung 1210 Wien Floridsdorf ✓ Alte Donau ✓ Stammersdorf ✓ Schnell & zuverlässig. Kostenlose Besichtigung ☎ +43 660 200 5610',
    shortSummary: 'Hausräumung im zweitgrößten Bezirk nördlich der Donau. Stammersdorf, Alte Donau mit Erfahrung in Einfamilienhäusern.',
    landmarks: ['Alte Donau', 'Bisamberg', 'Stammersdorf', 'Donauinsel'],
    neighborhoods: ['Floridsdorf', 'Stammersdorf', 'Strebersdorf', 'Jedlesee', 'Schwarze Lackenau'],
    characteristics: [
      'Zweitgrößter Bezirk',
      'Ländlicher Charakter',
      'Viele Einfamilienhäuser',
      'Weinbaugebiet Stammersdorf',
      'Naherholungsgebiet Alte Donau',
    ],
    serviceAreas: ['Floridsdorfer Hauptstraße', 'Brünner Straße', 'Stammersdorfer Straße', 'Leopoldauer Straße', 'Alte Donau'],
    popularServices: [
      'Hausräumung',
      'Wohnungsentrümpelung',
      'Kellerentrümpelung',
      'Gartenentrümpelung',
      'Dachbodenentrümpelung',
      'Verlassenschaftsentrümpelung',
    ],
    whyChooseUs: [
      'Kenntnis der weitläufigen Gebiete',
      'Erfahrung mit Einfamilienhäusern',
      'Gartenentrümpelungen',
      'Faire Preise',
      'Zuverlässige Anfahrt auch in Randlagen',
    ],
    faq: [
      {
        question: 'Kommen Sie auch nach Stammersdorf?',
        answer: 'Ja, wir decken den gesamten 21. Bezirk ab, auch die Randlagen wie Stammersdorf.',
      },
      {
        question: 'Können Sie auch Einfamilienhäuser entrümpeln?',
        answer: 'Ja, wir haben umfangreiche Erfahrung mit Hausräumungen und Gartenentrümpelungen.',
      },
      {
        question: 'Wie gehen Sie mit Gartenhäusern um?',
        answer: 'Wir entrümpeln auch Gartenhäuser, Schuppen und Außenbereiche komplett.',
      },
    ],
  },
  '1220-wien-donaustadt': {
    name: 'Donaustadt',
    postalCode: '1220',
    slug: '1220-wien-donaustadt',
    description: 'Die Donaustadt ist der flächenmäßig größte Bezirk Wiens mit der Donau City, Kaisermühlen und der Seestadt Aspern. Ein moderner, wachsender Bezirk mit Zukunft.',
    metaDescription: 'Entrümpelung 1220 Wien Donaustadt ✓ Donau City ✓ Seestadt ✓ Kagran ✓ Professionell & schnell. Kostenlose Beratung ☎ +43 660 200 5610',
    shortSummary: 'Moderne Entrümpelung im größten Bezirk. Donau City, Seestadt Aspern mit Erfahrung in Neubauten und großer Kapazität.',
    landmarks: ['Donau City', 'UNO City', 'Seestadt Aspern', 'Donauturm', 'Alte Donau'],
    neighborhoods: ['Kaisermühlen', 'Kagran', 'Stadlau', 'Aspern', 'Essling', 'Hirschstetten'],
    characteristics: [
      'Größter Bezirk Wiens',
      'Moderne Stadtentwicklung',
      'Seestadt Aspern',
      'Viele Neubauprojekte',
      'Donaunahe Lage',
    ],
    serviceAreas: ['Wagramer Straße', 'Donaustadtstraße', 'Erzherzog-Karl-Straße', 'Donau City', 'Seestadt Aspern'],
    popularServices: [
      'Wohnungsentrümpelung',
      'Neubau-Entrümpelung',
      'Büroräumung',
      'Kellerentrümpelung',
      'Geschäftsentrümpelung',
      'Haushaltsauflösung',
    ],
    whyChooseUs: [
      'Erfahrung mit Neubauten',
      'Kenntnis der modernen Stadtteile',
      'Große Kapazität',
      'Flexible Termine',
      'Professionelle Abwicklung',
    ],
    faq: [
      {
        question: 'Arbeiten Sie auch in der Seestadt?',
        answer: 'Ja, wir sind im gesamten 22. Bezirk tätig, auch in den neuen Stadtteilen.',
      },
      {
        question: 'Können Sie auch große Neubauwohnungen entrümpeln?',
        answer: 'Ja, wir haben Erfahrung mit modernen Wohnungen und Objekten jeder Größe.',
      },
      {
        question: 'Gibt es Zufahrtsbeschränkungen in der Donau City?',
        answer: 'Wir kennen die Zufahrtsmöglichkeiten und organisieren bei Bedarf Genehmigungen.',
      },
    ],
  },
  '1230-wien-liesing': {
    name: 'Liesing',
    postalCode: '1230',
    slug: '1230-wien-liesing',
    description: 'Liesing ist der südlichste Bezirk Wiens mit dem Liesing-Fluss, Wohn- und Industriegebieten und viel Grünraum. Ein Bezirk der Kontraste zwischen Natur und Urbanität.',
    metaDescription: 'Entrümpelung 1230 Wien Liesing ✓ Atzgersdorf ✓ Erlaa ✓ Mauer ✓ Professionell & fair. Kostenlose Besichtigung ☎ +43 660 200 5610',
    shortSummary: 'Entrümpelung im südlichsten Bezirk. Rodaun, Kalksburg bis Erlaa mit Erfahrung in Einfamilienhäusern und Industrie.',
    landmarks: ['Perchtoldsdorfer Heide', 'Maurer Wald', 'Liesing-Fluss', 'Himmelhof'],
    neighborhoods: ['Liesing', 'Atzgersdorf', 'Erlaa', 'Mauer', 'Rodaun', 'Kalksburg'],
    characteristics: [
      'Südlichster Bezirk',
      'Mix aus Wohnen und Industrie',
      'Viel Grünraum',
      'Einfamilienhäuser',
      'Nähe zum Wienerwald',
    ],
    serviceAreas: ['Breitenfurter Straße', 'Ketzergasse', 'Perfektastraße', 'Erlaaer Straße', 'Kalks burger Straße'],
    popularServices: [
      'Hausräumung',
      'Wohnungsentrümpelung',
      'Kellerentrümpelung',
      'Gartenentrümpelung',
      'Lagerentrümpelung',
      'Verlassenschaftsentrümpelung',
    ],
    whyChooseUs: [
      'Kenntnis der weitläufigen Gebiete',
      'Erfahrung mit Einfamilienhäusern',
      'Garten- und Außenbereich-Expertise',
      'Flexible Anfahrt',
      'Faire Preise',
    ],
    faq: [
      {
        question: 'Kommen Sie auch nach Rodaun oder Kalksburg?',
        answer: 'Ja, wir decken den gesamten 23. Bezirk ab, auch die Randlagen.',
      },
      {
        question: 'Können Sie auch Industrieobjekte entrümpeln?',
        answer: 'Ja, wir haben Erfahrung mit Gewerbe- und Industrieobjekten.',
      },
      {
        question: 'Wie schnell können Sie in Liesing sein?',
        answer: 'Wir streben schnelle Termine an, meist innerhalb von 1-2 Tagen.',
      },
    ],
  },
};

export const getAllDistricts = (): DistrictInfo[] => {
  return Object.values(viennaDistricts);
};

export const getDistrictBySlug = (slug: string): DistrictInfo | null => {
  return viennaDistricts[slug] || null;
};
